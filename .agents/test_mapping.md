...
- Seit `V92 92.5.1` ist die Budget-Baseline fuer Folgeblocks konkret eingefroren: `window.GAME_INSTANCE` darf nur in `src/core/AppInitializer.js` und `src/core/main.js` verbleiben; `GameRuntimePorts`-Fallbacks duerfen nur in `src/shared/runtime/GameRuntimePorts.js` und dem dokumentierten Restcaller `src/ui/MatchFlowTransitionHotspots.js` vorkommen. Jede Ausweitung verlangt zuerst `architecture-guard`.
...
- `src/core/AppInitializer.js` -> `npm run check:architecture:boundaries && npm run check:architecture:ratchet`
- `src/core/main.js` -> `npm run check:architecture:boundaries && npm run check:architecture:ratchet`
