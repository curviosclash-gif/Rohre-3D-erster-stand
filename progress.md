...
2026-03-25 (Cinematic Video-Perspektive + MP4-Priorisierung)
- Nutzerwunsch umgesetzt: Perspektiv-Modi wirken im Recording-Shorts-Pfad, nicht im Live-Gameplay.
- `src/core/renderer/RecordingCapturePipeline.js`:
  - `classic` bleibt im Shorts-Pfad neutral (kein Orbit-Style).
  - `cinematic_soft` -> `cinematic`, `cinematic_action` -> `action` fuer Video-Capture.
- `src/core/renderer/CameraRigSystem.js`: Perspective-Settings triggern Timing-Reset nur noch, wenn Live-Perspective aktiv ist; im Runtime-Pfad (`livePerspectiveEnabled=false`) keine Nebenwirkung.
- MP4-Priorisierung erweitert:
  - `src/core/recording/MediaRecorderSupport.js` priorisiert jetzt MP4-MIME-Candidates fuer MediaRecorder vor WebM.
- UI-Text auf Video-Scope geschärft:
  - `index.html`, `src/ui/UIManager.js`, `src/ui/menu/MenuGameplayBindings.js` -> Label/Hint/Toast von "Perspektive Normal" auf "Video-Perspektive".
- Neue Regressionstests:
  - `tests/gpu.spec.js` -> `T33d` (classic neutral, soft/action distinct, live camera unchanged).
- Verifikation:
  - `npx eslint src/core/renderer/RecordingCapturePipeline.js src/core/renderer/CameraRigSystem.js src/core/recording/MediaRecorderSupport.js src/ui/UIManager.js src/ui/menu/MenuGameplayBindings.js tests/core.spec.js tests/gpu.spec.js` PASS.
  - Lokale Node-Assertions (RecordingCapturePipeline-Style-Mapping + MediaRecorder-MP4-Prioritaet) PASS.
