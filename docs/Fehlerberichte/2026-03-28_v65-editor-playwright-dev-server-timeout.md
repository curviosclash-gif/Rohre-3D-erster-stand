...
- Tatsaechliches Verhalten:
  ...
  - Lock-freie Lokal-Konfiguration (`playwright.local.repro.config.mjs`) startet zwar Vite, aber HTTP-Requests auf `127.0.0.1:<port>` liefern in diesem Lauf 0 Bytes bis zum Timeout

## Reproduktion

1. `TEST_PORT=5291 npx playwright test --config playwright.local.repro.config.mjs tests/editor-map-ui.spec.js`
...
- Ansatz: Lock-freier Lauf ueber `playwright.local.repro.config.mjs`
...
- Editor-Spec erneut via `playwright.local.repro.config.mjs` oder stabilem vorhandenen Dev-Server laufen lassen
