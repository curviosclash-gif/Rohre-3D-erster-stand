...
Stand: 2026-04-13 (Audit-Nachtrag: V71 fuehrt getrackte Ignore-Artefakte als Restgate-Risiko, V77/V81 fuehren den Expertenlogin als Dev-only-/Surface-Risiko, und `docs/plaene/neu/Feature_Toolchain_Security_Dependency_Upgrade_2026-04-10.md` enthaelt einen Intake-Draft fuer Dependency-Security. V87, V88 und V89 sind blockerfest abgeschlossen; V89 spiegelt Browser-, Desktop- und Contract-Layer jetzt widerspruchsfrei in Mapping, Referenzdoku und Gate-Reihenfolge: `desktop-smoke` bleibt das primaere Desktop-Hauptsignal, `browser-compat` der nachgelagerte Web-/Demo-Layer und `node-contract` traegt weiterhin nur den blockerfest dokumentierten Restfehler `T97`, wodurch `test:core` aktuell allein am Contract-Layer haengt. V72 steht nach 72.4.2 in 72.5; V85 steht nach 85.2.2 in 85.3.)
...
| V85 | Persistence-, Content-Contracts und Schema-Migrationen | active | P2 | Agent-Codex | V83.99 | 85.3 | `docs/plaene/aktiv/V85.md` |
...
| Agent-B | V72 | 2026-04-02 | active | `72.4.2` abgeschlossen; `72.5.1` gezielte Regressionstests fuer Capability-, Gate- und Portal-Vertraege steht an |
...
| Agent-Codex | V85 | 2026-04-12 | active | `85.2.2` abgeschlossen; als naechstes `85.3.1` Maps, Runtime-Presets, Editor-Templates und weitere Content-Familien ueber gemeinsame Descriptor-Vertraege modellieren |
...
3. `V85` weiter durch `85.3.1` ziehen, damit der Persistence-/Content-Vertragsrahmen nach V89 ohne Verifikationsdrift aufgebaut wird.
4. `V77` als naechsten Produkt-/Surface-Pfad anziehen, sobald ein freier Slot dafuer vorgesehen ist.
...
2. `V85` bleibt der naechste Daten-/Persistenzpfad, damit Folgeblocks auf demselben Vertragsrahmen aufsetzen.
...
Parallel nach `V72.99` moeglich: `V82 -> V76`; `V86` bevorzugt nach `V85`; `V81` zuletzt oder opportunistisch. Die desktop-first-Hauptgates aus `V89` sind jetzt die Baseline fuer Folgearbeit am Desktop-Hauptprodukt.
