...
Leitplanke 2026-04-15 (Quelle: `V92 92.5.1`, Referenz 4.5.14): `V81` liest Runtime- oder Session-Zustand nicht ueber `window.GAME_INSTANCE`, `window.GAME_RUNTIME` oder direkte `GameRuntimePorts`-Fallback-Helfer. Die eingefrorene Restnische bleibt bei Bootstrap (`src/core/AppInitializer.js`, `src/core/main.js`) und `src/shared/runtime/GameRuntimePorts.js`/`src/ui/MatchFlowTransitionHotspots.js`; Developer-Tooling konsumiert stattdessen Bridge-, Store-, Snapshot- und Capability-Pfade.
...
- [ ] 81.2.6 Expertenlogin-/Dev-Mode-Zugang mit V77 abgleichen und den bestehenden clientseitigen Passwortpfad entweder als rein lokale UX-Grenze dokumentieren oder durch einen expliziten Desktop-Capability-Check ersetzen.
