...
- [ ] DoD.8 Neue Tuning-Integrationen folgen der `V91`-Ratchet-Leitplanke und fuehren keine neuen Legacy-Reads auf `game.runtimeBundle`, `game.runtimeFacade`, `window.GAME_RUNTIME`, `curviosApp`/`__CURVIOS_APP__` oder `getActiveRuntimeConfig` ausserhalb der vorgesehenen Config-/Settings-Adapter ein.
