...
Ein kleines, versioniertes und pruefbares Agent-Memory im Repo sowie eine spaetere, klar begrenzte Ruflo-Anbindung fuer Review und Orchestrierung planen, ohne Knowledge Graph, Masterplan, Changelog oder Governance-Regeln zu ersetzen.
...
- Die externe Repo `pogii123/obsidian-agentic-second-brain` wurde am 2026-06-04 als Quelleninput fuer ein Markdown-basiertes Agent-Grounding-Vertrauensmodell bewertet. Uebernommen wird daraus nur das Trust-Modell, nicht die Vault-/PARA-Struktur.
...
## Obsidian-Trust-Modell als Quelleninput

Die Obsidian-Vorlage ist kein Curvios-Tool und keine neue Repo-Struktur. Sie liefert aber ein nuetzliches Vertrauensmodell fuer Agent-Memory:

| Konzept | V122-Entscheidung | Curvios-Schnitt |
| --- | --- | --- |
| `system_of_record` | `adopt limited` | Memory-Eintraege und Mirror-Hinweise nennen, welche Curvios- oder externe Quelle den Fakt besitzt. |
| `verification` / `review_after` / `last_synced` | `adopt limited` | Memory kann `unverified`, `generated`, `verified` oder `stale` markieren und muss ablaufende Quellen sichtbar machen. |
| External Mirrors | `adopt concept` | Externe Systeme besitzen ihren STATE; Curvios besitzt nur durable Knowledge, Entscheidungen und validierte Handoffs. |
| `source:generated` Bloecke | `adopt limited` | Nur fuer spaetere Mirror-/Sync-Dateien unter `docs/agent-memory/**`; nicht global fuer aktive Plaene oder Changelog. |
| Vault Health | `adopt concept` | `memory:check` wird der CLI-Health-Report fuer stale, unverified, source-drift und Secret-Funde. |
| PARA-/Obsidian-Ordnerstruktur | `reject` | Keine `10 Projects`-/`20 Areas`-/Vault-Struktur im Hauptrepo; Curvios bleibt VXX-/Graph-/Code-first. |
| Bash-Hook | `reject` | Windows-/Repo-taugliche Umsetzung nur als Node-/npm-Gate. |
| `llms.txt` | `defer` | Hoechstens spaeter als Adapter/Export; `AGENTS.md`, Rules, Workflows und `check:agent-context` bleiben fuehrend. |

Leitregel fuer die Umsetzung:

```text
Obsidian wird nicht als Vault-Struktur uebernommen, sondern als Vertrauensmodell fuer Curvios-Agent-Memory adaptiert.
```
...
`system_of_record` muss ausdruecken, wem der belegte Fakt gehoert, zum Beispiel `curvios-plan`, `curvios-graph`, `curvios-changelog`, `git`, `ruflo-cache`, `github` oder ein spaeter konkret benannter externer Mirror.
...
Wenn `review_after` ueberschritten ist, bleibt der Eintrag lesbar, wird aber von `memory:check` als stale gemeldet und darf nicht als starke Entscheidungsgrundlage dienen, bis Quelle oder Hash erneut geprueft wurden.
...
- `system_of_record`, `verification`, `last_synced` und `review_after` sind typ- und statuskonsistent.
- `verification: verified` braucht nachvollziehbare Quellen und `verified_by`; `verification: generated` braucht `last_synced`.
- abgelaufene `review_after`-Daten werden als stale Health-Warnung gemeldet.
...
- spaetere Mirror-Dateien mit `source:generated`-Bloecken haben gepaarte Start-/End-Marker mit `id=`, `tool=` und `source=`; diese Pruefung wird als Node-Check umgesetzt, nicht als Bash-Hook.
...
| External Mirror / Task Routing | `adopt concept` | Ruflo/GitHub/Tracker liefern nur Kontext oder proposed Findings; Curvios-Quellen entscheiden |
...
- [ ] DoD.13 Das Obsidian-Trust-Modell ist Curvios-nativ umgesetzt: Metadaten, External-Mirror-Regel, Health-Report und Node-Provenance-Check sind vorhanden oder bewusst vertagt; keine Vault-/PARA-Struktur, kein Bash-Hook und keine konkurrierende Agent-Governance wurden eingefuehrt.
...
- [ ] 122.1.7 Obsidian-Second-Brain als externe Quellenanalyse festhalten: `adopt limited` fuer Trust-Metadaten, External Mirrors, Health-Check und generated-block-Provenance; `reject` fuer Vault-/PARA-Struktur, Bash-Hook und konkurrierende Agent-Governance.
...
- [ ] 122.2.6 Trust-Metadaten aus dem Obsidian-Modell Curvios-nativ definieren: `system_of_record`, `verification`, `last_synced`, `review_after`, `source_hash`; Status und Verification nicht vermischen.
- [ ] 122.2.7 External-Mirror-Vertrag fuer spaetere Ruflo-/GitHub-/Tracker-Kontexte dokumentieren: Externes System besitzt STATE, Curvios besitzt durable Knowledge und validierte Entscheidungen.
...
- [ ] 122.3.6 `memory:check` als Health-Report ausbauen: stale Quellen, unverified Eintraege, `source_hash`-Drift, fehlendes `verified_by`, Secret-Muster, zu lange Summaries und generated-block-Provenance fuer Mirror-Dateien.
...
- [ ] 122.3a.6 Rollen- und Ruflo-Ausgaben duerfen nur Findings oder `proposed` Memory liefern; `verified` bleibt Curvios-Review-/Gate-Entscheidung.
...
- [ ] 122.6.8 Ruflo wie einen External Mirror behandeln: STATE und Cache bleiben extern, Curvios uebernimmt nur bewusst gepruefte durable Knowledge-Eintraege.
...
- [ ] 122.7.6 `llms.txt`-aehnlichen Root-Index nur als spaeteren Adapter/Export bewerten; keine neue Wahrheitsschicht neben `AGENTS.md`, Rules, Workflows und `check:agent-context`.
...
| Obsidian-Vault-Struktur erzeugt zweite Plan-/Wissensordnung | hoch | Nur Trust-Modell uebernehmen; keine PARA-Ordner, kein Obsidian-Vault im Hauptrepo |
| Bash-Provenance-Hook ist unter Windows nicht nutzbar | mittel | Provenance- und Health-Pruefung als Node-/npm-Gate bauen |
...
| Trust-Health | `memory:check` meldet stale, unverified, generated, verified und source-drift nachvollziehbar |
