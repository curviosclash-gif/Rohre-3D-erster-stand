...
scope_reference_files:
  - docs/plaene/neu/Feature_Finding_Plan_Doku_Drift_Automatisierung_V141.md
verification:
...
- Alte QA-/Backlog-Snapshots als aktiv, historisch oder stale klassifizierbar machen, ohne sie mechanisch umzuschreiben.
...
- [x] DoD.1 Manuelle Finding-Entscheidungen sind von generiertem Status getrennt, ohne neue Schatten-Wahrheit. (abgeschlossen: 2026-06-09; evidence: `finding-decisions.v1` besitzt nur manuelle Entscheidungsfelder; `open-findings-index.v1` bleibt `generated-signals-only`)
- [x] DoD.2 `open-findings-index.v1` ist dokumentiert, generierbar und gegen einfache Drift-Faelle getestet. (abgeschlossen: 2026-06-09; evidence: `npm run findings:index:build` -> PASS; `node --test tests/open-findings-index.contract.test.mjs` -> 6 PASS)
- [x] DoD.3 `findings:check` erkennt mindestens: stale owning block, abgelaufene Wiedervorlage, fehlende Datei, Changelog-erledigt-aber-offen und Zuordnung/Tabelle-Widerspruch. (abgeschlossen: 2026-06-09; evidence: `tests/open-findings-index.contract.test.mjs` -> 6 PASS; Repo-Pilot -> 10 erwartete WARN-Signale)
- [x] DoD.4 `P21` bleibt als accepted-risk/Wiedervorlage modelliert und wird nicht automatisch geschlossen. (abgeschlossen: 2026-06-09; evidence: Generated-Index `P21` -> `accepted-risk`, `review_after=2026-06-17`, `drift=[]`)
- [x] DoD.5 Plan-/Changelog-Abgleich meldet fehlende oder widerspruechliche `*.99`-Abschlussnotizen. (abgeschlossen: 2026-06-09; evidence: `npm run plan:changelog:check` -> PASS mit 8 WARN und 25 historischen INFO)
- [x] DoD.6 `.agents/test_mapping.md` wird gegen existierende Scripts/Pfade und neue Testmuster geprueft, ohne Testauswahl automatisch umzuschreiben. (abgeschlossen: 2026-06-09; evidence: `npm run test-mapping:check` -> PASS, 0 WARN; Mapping-Datei unveraendert)
- [x] DoD.7 QA-/Release-/Backlog-Snapshots erhalten Freshness-Signale oder klare Statuslabels. (abgeschlossen: 2026-06-09; evidence: Release-Roadmap per User-Entscheid entfernt; Backlog -> `historical`; QA-Checkliste -> `stale`; `npm run docs:sync` -> PASS)
- [x] DoD.8 Warn-/Fehlerklassen sind dokumentiert; harte Gate-Integration ist bewusst entschieden oder deferred. (abgeschlossen: 2026-06-09; evidence: Gate-Matrix in 141.6; `npm run docs:check` -> PASS mit WARN/INFO-only Drift-Pilot)
- [x] DoD.9 `plan:check`, `plan:index:check`, `plan:context:check`, `docs:sync` und `docs:check` sind gruen oder blockerfest dokumentiert. (abgeschlossen: 2026-06-09; evidence: `npm run plan:check`, `npm run plan:index:check`, `npm run plan:context:check`, `npm run docs:sync`, `npm run docs:check` -> PASS)
- [x] DoD.99 Abschlussnotiz nennt Pilot-Umfang, nicht automatisierte Entscheidungen, Restrisiko und naechsten moeglichen Gate-Schritt. (abgeschlossen: 2026-06-09; evidence: Abschnitt `Abschlussnotiz 2026-06-09`)
...
- [x] 141.1.1 Aktuelle `Open_Findings.md`-Eintraege in Kategorien schneiden: `manual-decision`, `status-signal`, `owner-mapping`, `stale-candidate`. (abgeschlossen: 2026-06-09; evidence: Generated-Index trennt Decision-Overlay, Master-Signale, Markdown-Mappings und Drift-Codes)
- [x] 141.1.2 Schema fuer `docs/prozess/finding-decisions.json` festlegen: `id`, `status`, `owner_block`, `severity`, `review_after`, `reason`, `manual_override`. (abgeschlossen: 2026-06-09; evidence: `docs/prozess/finding-decisions.json`)
- [x] 141.1.3 Schema fuer `docs/generated/open-findings-index.json` festlegen: Quellen, Signale, Drift-Klasse, Confidence und Wiedervorlage. (abgeschlossen: 2026-06-09; evidence: `npm run findings:index:build` -> `docs/generated/open-findings-index.json`)
- [x] 141.1.4 `P21`, `P47` und `P48` als Pilotfaelle modellieren, ohne ihren fachlichen Status automatisch zu aendern. (abgeschlossen: 2026-06-09; evidence: `P21=accepted-risk`; `P47/P48=open` mit done-owner WARN)
...
- [x] 141.2.1 `scripts/build-open-findings-index.mjs` implementieren: Entscheidungen, Master/Plan-Index, VXX-Dateien und Open-Findings-Tabelle lesen. (abgeschlossen: 2026-06-09; evidence: `npm run findings:index:build` -> PASS, 6 Finding-IDs)
- [x] 141.2.2 `scripts/check-open-findings.mjs` implementieren: Drift-Codes fuer `owner-block-done`, `review-after-due`, `missing-file`, `changelog-resolved-open`, `mapping-table-mismatch`. (abgeschlossen: 2026-06-09; evidence: Contract-Tests -> 6 PASS; Repo-Pilot -> WARN-only)
- [x] 141.2.3 `package.json` um `findings:index:build` und `findings:check` ergaenzen. (abgeschlossen: 2026-06-09; evidence: beide npm-Kommandos -> PASS)
- [x] 141.2.4 Contract-Test fuer mindestens drei positive und drei negative Drift-Faelle ergaenzen. (abgeschlossen: 2026-06-09; evidence: `tests/open-findings-index.contract.test.mjs` -> 6 PASS)
...
- [x] 141.3.1 Regeln definieren: `*.99` im Master braucht Abschlussnotiz; Abschlussnotiz muss Block/Phase/Datum/Evidence oder Not-checked nennen. (abgeschlossen: 2026-06-09; evidence: `scripts/check-plan-changelog-drift.mjs`)
- [x] 141.3.2 Offene Findings im owning Scope gegen Blockstatus und Changelog abgleichen. (abgeschlossen: 2026-06-09; evidence: `open-finding-owner-done` fuer P14/P45/P46/P47/P48)
- [x] 141.3.3 `scripts/check-plan-changelog-drift.mjs` als separaten Check oder schmalen `validate-umsetzungsplan`-Integrationspunkt umsetzen. (abgeschlossen: 2026-06-09; evidence: `npm run plan:changelog:check` -> PASS)
- [x] 141.3.4 Warnklasse kalibrieren: keine harten Failures fuer historische Altbloecke ohne expliziten V141-Gate-Beschluss. (abgeschlossen: 2026-06-09; evidence: historische Closure-Formatluecken -> INFO; Status-/Finding-Widersprueche -> WARN; Exit 0)
...
- [x] 141.4.1 Genannte npm-Scripts gegen `package.json` pruefen. (abgeschlossen: 2026-06-09; evidence: `npm run test-mapping:check` -> 0 fehlende Scripts)
- [x] 141.4.2 Genannte Pfade und Globs gegen Repo-Dateien pruefen; absichtliche Pattern klar erlauben. (abgeschlossen: 2026-06-09; evidence: `check-test-mapping-drift.mjs` + Contract-Fall `missing-mapped-path`)
- [x] 141.4.3 Neue `tests/*.contract.test.mjs` und kanonische Runner-Surfaces gegen Mapping-Sichtbarkeit pruefen. (abgeschlossen: 2026-06-09; evidence: Contract-Catch-all und vier kanonische Runner sichtbar)
- [x] 141.4.4 Drift-Check mit `WARN` starten; harte Fehler nur fuer geloeschte Scripts/Pfade nach separater Entscheidung. (abgeschlossen: 2026-06-09; evidence: `npm run test-mapping:check` -> `pilot=warn-only`)
...
- [x] 141.5.1 `docs/prozess/Backlog.md` und `docs/qa/Manuelle_Testcheckliste_Spiel.md` klassifizieren: aktiv, historisch, draft oder stale. (abgeschlossen: 2026-06-09; evidence: Backlog=`historical`; QA-Checkliste=`stale`)
- [x] 141.5.2 `scripts/docs-freshness.mjs` um konservative Warnungen fuer alte Stand-Daten und offene Checklisten erweitern. (abgeschlossen: 2026-06-09; evidence: `npm run docs:sync` -> `snapshot-warnings=2`)
- [x] 141.5.3 Keine automatische Textkorrektur in QA-/Backlog-Dateien ohne User-Gate; nur Statussignal oder expliziter Label-Vorschlag. (abgeschlossen: 2026-06-09; evidence: beide Quelldateien unveraendert; nur Statusreport geschrieben)
- [x] 141.5.4 `docs/prozess/Dokumentationsstatus.md` zeigt die neuen Freshness-Warnungen kompakt. (abgeschlossen: 2026-06-09; evidence: `npm run docs:sync` -> PASS)

User-Entscheid 2026-06-09: Die veraltete Release-Roadmap wurde geloescht und ist kein V141-Freshness-Consumer mehr.
...
- [x] 141.6.1 Gate-Matrix erstellen: `findings:check`, `plan:check`, `docs:check`, `gates:pre-commit`. (abgeschlossen: 2026-06-09; evidence: `drift:check` buendelt drei WARN-Piloten; `docs:check` ruft ihn additiv auf; `plan:check` bleibt unveraendert)
- [x] 141.6.2 WARN/FAIL-Grenzen je Drift-Code festlegen und P1-/Security-Faelle gesondert behandeln. (abgeschlossen: 2026-06-09; evidence: Driftcodes -> WARN/INFO; Parser-/Schema-/IO-Fehler -> FAIL; P21 bleibt accepted-risk und wird nur nach `review_after` warnend faellig)
- [x] 141.6.3 Entscheiden, ob `Open_Findings.md` manuelle Entscheidungsschicht bleibt oder spaeter als generierte Ansicht markiert werden soll. (abgeschlossen: 2026-06-09; evidence: Markdown bleibt kanonisches Inventar; JSON besitzt nur explizite manuelle Entscheidungsfelder; Generated-Index hat keine Autoritaet)
- [x] 141.6.4 Folge-Intake fuer einen echten Source-of-Truth-Wechsel nur anlegen, wenn Pilot-Evidence zeigt, dass der Generator stabiler ist als manuelle Pflege. (abgeschlossen: 2026-06-09; evidence: deferred/no-op; kein Folge-Intake, da 10 Finding-WARN und historische Kalibrierung noch menschliche Entscheidung brauchen)
...
- [x] 141.99.1 `npm run findings:index:build` ist gruen und erzeugt deterministisches Output. (abgeschlossen: 2026-06-09; evidence: zwei Builds -> identischer SHA-256; `npm run findings:index:build` -> PASS)
- [x] 141.99.2 `npm run findings:check` ist gruen oder meldet erwartete, dokumentierte Warnungen. (abgeschlossen: 2026-06-09; evidence: PASS mit 10 erwarteten WARN-Signalen, `P21` ohne Drift)
- [x] 141.99.3 `npm run plan:check`, `npm run plan:index:check` und `npm run plan:context:check` sind gruen. (abgeschlossen: 2026-06-09; evidence: alle drei Kommandos -> PASS)
- [x] 141.99.4 `npm run docs:sync` und `npm run docs:check` sind gruen oder blockerfest dokumentiert. (abgeschlossen: 2026-06-09; evidence: beide Kommandos -> PASS; Snapshot-Warnungen bleiben nicht blockierend)
- [x] 141.99.5 Abschlussnotiz nennt: manuelle Entscheidungen, generierte Signale, harte Nicht-Ziele, Warn-/Fail-Politik, Restrisiko und naechsten Schritt. (abgeschlossen: 2026-06-09; evidence: Abschnitt `Abschlussnotiz 2026-06-09`)

## Abschlussnotiz 2026-06-09

- Pilot-Umfang: manuelle Decision-Overlays fuer `P21`, `P47`, `P48`; deterministischer Open-Findings-Index; Finding-, Plan-/Changelog-, Testmapping- und Snapshot-Freshness-Checks.
- Autoritaet: `Open_Findings.md` bleibt kanonisches Inventar, `finding-decisions.json` besitzt nur explizite manuelle Entscheidungsfelder, Generated-Indizes und Driftchecks liefern keine automatische Schliess- oder Schreibautoritaet.
- WARN-/FAIL-Politik: fachliche Driftcodes starten als WARN, historische Closure-Formatluecken als INFO; Parser-, Schema- und IO-Fehler bleiben FAIL. `P21` bleibt bis zur Wiedervorlage 2026-06-17 `accepted-risk`.
- Harte Nicht-Ziele: kein Finding automatisch geschlossen, kein Master-/VXX-Writer, kein Source-of-Truth-Wechsel, keine Runtime-/Gameplay-/Security-Parameter geaendert.
- Restrisiko: P14/P45/P46/P47/P48 sowie drei historische Master-/Aktivplan-Statussignale brauchen menschliche Triage; die QA-Checkliste bleibt mit 241 offenen Punkten stale.
- Naechster moeglicher Gate-Schritt: V142 darf Graph-Generated-Gates auf Basis der kalibrierten WARN-Klassen haerten; ein Finding-Source-of-Truth-Wechsel bleibt deferred.
