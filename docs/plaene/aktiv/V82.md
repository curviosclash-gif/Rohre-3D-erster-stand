...
- **Snapshot**: HUD-Updates lesen `match_flow_snapshot` oder `session_runtime_snapshot` statt `getActiveRuntimeConfig()` oder `window.GAME_RUNTIME`.
- **Sunset alter Pfade**: Keine neuen Aufrufer fuer `game.runtimeBundle`, `game.runtimeFacade`, `window.GAME_RUNTIME`, `curviosApp`/`__CURVIOS_APP__` oder `getActiveRuntimeConfig` (ausserhalb Config/Settings); `check:architecture:boundaries` blockiert neue Violations automatisch.
