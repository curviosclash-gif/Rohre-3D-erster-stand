...
- V141-Abschlussanalyse 2026-06-09: `node scripts/query-knowledge-graph.mjs open-deps V141 --json -> WARN`: Master und `plan-index` zeigen V141-Dependencies als erledigt, aber Graph-Kanten melden mehrere erfuellte Mehrfach-Abhaengigkeiten als offen. Vermutete Ursache ist, dass `scripts/build-knowledge-graph.mjs` in der Master-Abhaengigkeitstabelle kompakte Zellen wie `V116.99,V117.99,V119.99,V123.99,V138.99` nur als erstes Token materialisiert. V142 soll diesen Parser-/Contract-Fall aufnehmen.
...
- [ ] DoD.8 Contract-Tests decken positiven Sync, fehlenden Generated-Sync, unrelated staged Files, untracked-noise und Mehrfach-Dependency-Zellen aus der Master-Abhaengigkeitstabelle ab.
...
- [ ] 142.1.4 Baseline-Kommandos ausfuehren: `git status --short`, `npm run graph:check`, `node scripts/query-knowledge-graph.mjs scope-collisions --json`, `node scripts/query-knowledge-graph.mjs open-deps V141 --json`.
- [ ] 142.1.5 Master-Abhaengigkeitstabellen mit Mehrfach-Tokens als Regression-Fall erfassen: `V141 | V116.99,V117.99,V119.99,V123.99,V138.99 | hard | ja` darf im Graph keine falschen offenen `open-deps` fuer die Folge-Tokens erzeugen.
...
- [ ] 142.4.5 Dependency-Metadata-Baseline pruefen: `depends_on`-Kanten aus kompakten Master-Tabellenzellen behalten `hard`, `fulfilled` und `hint` fuer jedes Token oder melden eine eindeutige Parser-Warnung.
