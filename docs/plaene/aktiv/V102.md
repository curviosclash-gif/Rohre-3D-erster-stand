...
- [x] 102.1.1 `vite.config.js` Video-Save-Endpoint auf erlaubte Root begrenzen (`safeName`, canonical path guard, reject ausserhalb root). (abgeschlossen: 2026-05-12; evidence: test -> pass)
- [x] 102.1.2 `StartSetupUiOps` und `MatchFlowArcadeOverlayController` von untrusted `innerHTML` auf sichere DOM-Updates umstellen; der LAN-Discovery-Pfad bleibt bei `V99`. (abgeschlossen: 2026-05-12; evidence: test -> pass)
...
- [x] 102.2.1 `PlatformCapabilityRegistry` Build-Artefakt-Lesepfad ohne synchrones XHR aufloesen (lazy async cache oder bootstrap-injizierter Snapshot). (abgeschlossen: 2026-05-12; evidence: test -> pass)
- [x] 102.2.2 `preload`/Settings-Defaults-Read so umstellen, dass kein `sendSync` im rendererseitigen Verbrauchspfad notwendig ist. (abgeschlossen: 2026-05-12; evidence: test -> pass)
...
- [x] 102.3.1 `server/lan-signaling.js` Request-Body-Limits und parse-feste Fehlerpfade einfuehren. (abgeschlossen: 2026-05-12; evidence: test -> pass)
- [x] 102.3.2 Endpoint-Rollen-/Mutationsschutz mit bestehendem `V99`-Scope abgleichen, ohne parallelen Vertragszweig zu bauen. (abgeschlossen: 2026-05-12; evidence: test -> pass)
...
- [x] 102.4.1 `eslint`/Architektur-Checks fuer Security-/Hotspot-Regressionen schaerfen (z. B. no-untrusted-innerhtml-Regelpfad oder gleichwertiger Guard). (abgeschlossen: 2026-05-12; evidence: lint -> pass)
- [x] 102.4.2 Repo-Hygiene-Follow-up (`tmp`-/Archiv-Bloat) als klaren Retention-/Enttracking-Plan dokumentieren und in bestehende Cleanup-Streams einhaengen. (abgeschlossen: 2026-05-12; evidence: manual -> updated)
...
- [x] 102.99.1 Relevante Contract-/Targeted-Tests fuer Security-/Contract-Pfade sind gruen oder blockerfest dokumentiert. (abgeschlossen: 2026-05-12; evidence: test -> pass)
- [x] 102.99.2 `npm run plan:check`, `npm run docs:sync` und `npm run docs:check` sind gruen. (abgeschlossen: 2026-05-12; evidence: test -> pass)
- [x] 102.99.3 Keine neuen disallowed architecture edges oder Legacy-Surface-Budgetausweitungen im geaenderten Scope. (abgeschlossen: 2026-05-12; evidence: test -> pass)
